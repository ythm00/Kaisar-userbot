...
async def logo_gen(event):
    ...
    if not name:
        await eod(xx, "`Berikan kaisar sebuah nama!`")
    bg_, font_ = "", ""
    if event.reply_to_msg_id:
        ...
        async for i in ultroid.iter_messages(
            "@KaisarLogos", filter=InputMessagesFilterPhotos
        ):
            pics.append(i)
        id_ = random.choice(pics)
        bg_ = await id_.download_media()
        fpath_ = glob.glob("style/FontKaisar/*")
    ...
    if os.path.exists(font_):
        if not font_.startswith("style/FontKaisar"):
            os.remove(font_)