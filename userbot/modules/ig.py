...
async def insta(event):
    ...
    if not event.reply_to_msg_id:
        await event.edit("`Kaisar, Mohon Balas Ke Link Instagram`")
        return
    reply_message = await event.get_reply_message()
    if not reply_message.text:
        await event.edit("`Mohon Maaf Kaisar, Saya Membutuhkan Link Media Instagram Untuk Download`")
        return
    chat = "@SaveAsBot"
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await event.edit("`Kaisar, Mohon Buka Blokir` @SaveAsbot `Lalu Coba Lagi`")
            return
