                ...
                try:
                    ...
                    if BOTLOG:
                        await event.client.send_message(
                            BOTLOG_CHATID,
                            "Kaisar, Terjadi Masalah Saat Menghitung Private Message, Mohon Restart Bot!",
                        )
...
async def add_pmsg(cust_msg):
    ...
    if not PM_AUTO_BAN:
        return await cust_msg.edit("**Kaisar Anda Harus Menyetel** `PM_AUTO_BAN` **Ke** `True`")
    try:
    ...
    if event.is_private:
        if not pm_permit_sql.is_approved(chats.id):
            pm_permit_sql.approve(
                chats.id, "`Kaisar KEN KAN Telah Mengirimi Anda Pesan 😯`")
            await borg.send_message(
                chats, "**Menerima Pesan!, Pengguna Terdeteksi Adalah Kaisar KEN KAN**"
            )
